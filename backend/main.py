...
TOS_VERSION = "2026-06-24"          # bump when the Terms change to re-prompt acceptance
...
@app.get("/api/tos")
async def api_tos() -> dict:
    return {"version": TOS_VERSION}


@app.post("/api/signup")
async def api_signup(p: dict, request: Request):
    ...
    if not p.get("accept_tos"):
        return JSONResponse({"error": "กรุณายอมรับข้อกำหนดการใช้งานก่อนสมัคร"}, status_code=400)
    if db.user_by_email(email):
        return JSONResponse({"error": "อีเมลนี้ถูกใช้แล้ว — ลองเข้าสู่ระบบ"}, status_code=409)
    uid = db.user_create(email, pwd, name, tos_version=TOS_VERSION, tos_ip=_client_ip(request))
...
async def imageai_generate(p: dict) -> dict:
    prompt = (p.get("prompt") or "").strip()
    image = (p.get("image") or "").strip()   # optional base image (data URI) to edit
    if not prompt:
        return {"error": "ใส่คำอธิบายภาพก่อน (เช่น 'โลโก้ร้านกาแฟ มินิมอล โทนน้ำตาล')"
                         if not image else "บอกด้วยว่าต้องการแก้ภาพยังไง (เช่น 'เปลี่ยนพื้นหลังเป็นชายหาด')"}
    ...
    try:
        ...
        label = ("✏️ แก้ไข: " + prompt) if image else prompt
        tid = db.image_add(label, cfg["provider"], cfg["model"], im["mime"], ref)
...
_TERMS_BODY = """
<p>ข้อกำหนดนี้ใช้กับการใช้บริการแพลตฟอร์มของ {C} ("บริการ", "เรา") การสมัครหรือใช้บริการถือว่าคุณ ("ผู้ใช้")
ได้อ่าน เข้าใจ และยอมรับข้อกำหนดนี้ทั้งหมด</p>

<h2>1. การใช้งานอย่างถูกกฎหมาย</h2>
<p>คุณตกลงใช้บริการอย่างถูกต้องตามกฎหมายไทยและกฎหมายที่เกี่ยวข้องทุกฉบับ และปฏิบัติตามนโยบายของแพลตฟอร์มที่เชื่อมต่อ
(เช่น นโยบายโฆษณาและเนื้อหาของ Meta/Facebook, Google, TikTok) ตลอดเวลา</p>

<h2>2. การใช้งานที่ห้ามโดยเด็ดขาด</h2>
<p>ห้ามใช้บริการเพื่อสร้าง โปรโมต ยิงโฆษณา หรือเผยแพร่สิ่งต่อไปนี้:</p>
<ul>
 <li><b>การพนันทุกรูปแบบ</b> — เว็บ/แอปพนัน คาสิโน สลากผิดกฎหมาย เดิมพัน หรือการชักชวนเล่นพนัน</li>
 <li>สิ่งผิดกฎหมายอื่น ๆ เช่น ยาเสพติด ของผิดกฎหมาย การฟอกเงิน สินค้าละเมิดลิขสิทธิ์/ปลอม</li>
 <li>การหลอกลวง สแกม แชร์ลูกโซ่ การลงทุนหลอก หรือการเงินนอกระบบ</li>
 <li>เนื้อหาผิดกฎหมาย ลามกอนาจารผิดกฎหมาย ความรุนแรง การละเมิดสิทธิ์ผู้อื่น หรือข้อมูลเท็จที่ก่อความเสียหาย</li>
 <li><b>การหลบเลี่ยงการตรวจสอบ (cloaking)</b> หรือการหลอกระบบรีวิวของแพลตฟอร์มโฆษณา</li>
 <li>มัลแวร์ การเจาะระบบ หรือการละเมิดความปลอดภัยของผู้อื่น</li>
</ul>
<p>เราขอสงวนสิทธิ์ใช้ระบบอัตโนมัติและการตรวจสอบเพื่อปฏิเสธ/บล็อกเนื้อหาที่เข้าข่ายข้างต้น</p>

<h2>3. บัญชีและข้อมูลของคุณแยกต่างหาก</h2>
<p>แต่ละบัญชีผู้ใช้มีพื้นที่ข้อมูล คีย์ และการเชื่อมต่อ (รวมถึง Facebook App/บัญชีโฆษณา) เป็นของตนเอง
คุณรับผิดชอบบัญชีภายนอกที่นำมาเชื่อม คีย์ API ที่ใส่เอง และยืนยันว่าคุณมีสิทธิ์จัดการบัญชี/เพจ/บัญชีโฆษณานั้น
คุณต้องเก็บรหัสผ่านเป็นความลับ และรับผิดชอบทุกการกระทำภายใต้บัญชีของคุณ</p>

<h2>4. ความรับผิดชอบของผู้ใช้และการชดใช้ (Indemnification)</h2>
<p><b>เนื้อหา แคมเปญ และการกระทำทั้งหมดที่ทำผ่านบัญชีของคุณ เป็นความรับผิดชอบของคุณแต่เพียงผู้เดียว</b>
คุณตกลงปกป้อง ชดใช้ และทำให้ {C} รวมถึงผู้เกี่ยวข้อง พ้นจากความรับผิด การเรียกร้อง ค่าเสียหาย ค่าปรับ
ค่าทนาย และค่าใช้จ่ายใด ๆ ที่เกิดจาก (ก) การที่คุณใช้บริการผิดข้อกำหนดนี้หรือผิดกฎหมาย
(ข) เนื้อหา/โฆษณาที่คุณสร้างหรือเผยแพร่ (ค) การละเมิดสิทธิ์ของบุคคลที่สามหรือนโยบายแพลตฟอร์มภายนอก</p>

<h2>5. การระงับบัญชีและความร่วมมือทางกฎหมาย</h2>
<p>เราขอสงวนสิทธิ์ระงับหรือยุติบัญชีของคุณทันทีโดยไม่ต้องแจ้งล่วงหน้า หากพบหรือสงสัยว่ามีการใช้งานผิดข้อกำหนด
และเราจะให้ความร่วมมือกับเจ้าหน้าที่รัฐ/หน่วยงานบังคับใช้กฎหมายตามที่กฎหมายกำหนด รวมถึงการเปิดเผยข้อมูลที่จำเป็น</p>

<h2>6. เครื่องมือ AI เป็นเพียงตัวช่วย</h2>
<p>ฟีเจอร์ AI (เช่น การวิเคราะห์ภาพ การสร้างเนื้อหา คำแนะนำโฆษณา) เป็นเพียงตัวช่วย
ไม่รับประกันผลลัพธ์ ความถูกต้อง หรือการอนุมัติจากแพลตฟอร์มภายนอก คุณต้องตรวจสอบก่อนนำไปใช้</p>

<h2>7. ข้อจำกัดความรับผิด</h2>
<p>บริการให้ "ตามสภาพ" (as-is) เราไม่รับประกันว่าจะไม่มีข้อผิดพลาดหรือหยุดชะงัก และไม่รับผิดต่อความเสียหายใด ๆ
ที่เกิดจากการใช้บริการหรือบริการภายนอก เท่าที่กฎหมายอนุญาต</p>

<h2>8. การเปลี่ยนแปลง</h2>
<p>เราอาจปรับปรุงหรือยุติบริการ และอาจแก้ไขข้อกำหนดนี้ได้ โดยจะแจ้งวันที่ปรับปรุงล่าสุดไว้ด้านบน
การใช้บริการต่อหลังการแก้ไขถือว่าคุณยอมรับข้อกำหนดที่ปรับปรุงแล้ว</p>
"""
